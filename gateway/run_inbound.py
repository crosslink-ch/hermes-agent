...
class GatewayInboundMixin:
    ...
    async def _hm_clarify_reply(
        ...
        _text_outcome = _clarify_mod.attempt_text_response_for_session(_quick_key, _raw_clarify_reply, resolved=_clarify_resolution)
        if _text_outcome == _clarify_mod.TEXT_RESOLVED:
            ...
            if _clarify_adapter:
                ...
                if _clarify_resolution and getattr(type(_clarify_adapter), "send_clarify_resolution", None) is not None:
                    try:
                        await _clarify_adapter.send_clarify_resolution(
                            chat_id=source.chat_id, session_key=_clarify_resolution["session_key"],
                            request_id=_clarify_resolution["request_id"], response=_clarify_resolution["response"],
                        )
                    except Exception:
                        logger.debug("Failed to publish typed clarify resolution", exc_info=True)
            return ""
    ...
    def _classify_inbound_media(
        ...
        from gateway.run import _event_media_is_audio, _event_media_is_audio_file, _event_media_is_image, _event_media_is_stt_input
