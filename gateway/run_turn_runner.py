...
from gateway.run_progress import StructuredProgressMixin


class TurnRunner(StructuredProgressMixin):
    ...
    def progress_callback(self, event_type: str, tool_name: str = None, preview: str = None, args: dict = None, **kwargs):
        ...
        self._record_structured_tool_progress(
            event_type,
            tool_name,
            preview,
            **kwargs,
    ...
    def combined_tool_start_callback(self, call_id, tool_name, args):
        """Compose structured, voice-ack, and native task-card consumers."""
    ...
    def _attach_session_title_callback(self, agent, ctx) -> None:
        ...
        try:
            ...
            if source.platform == Platform.THECHAT and source.thread_id:
                callback = runner._make_thechat_session_title_callback(source, event_message_id=ctx.event_message_id)
                if callback is not None:
                    agent._on_session_title = lambda title, title_source="llm": title_source == "llm" and callback(title)
                return
    ...
    def _status_callback_sync(self, event_type: str, message: str) -> None:
        ...
        if prepared is None:
            ...
            return
        def present():
            if ctx.source.platform == Platform.THECHAT and ctx._structured_progress_supported:
                notice_type, notice_status = self._thechat_notice_event_shape(event_type)
    ...
    def _wire_turn_agent_callbacks(self, agent, turn_route, reasoning_config,
        ...
        agent.tool_start_callback = (
            ...
            if (ctx._structured_progress_supported or ctx._voice_ack_guild[0] is not None or ctx._native_slack_task_cards) else None
        )
        agent.tool_complete_callback = ctx.native_tool_complete_callback if (ctx._structured_progress_supported or ctx._native_slack_task_cards) else None
    ...
    def _ask_clarify_question(self, question, choices, multi_select, rearm: bool = True) -> tuple[str, bool]:
        ...
        clarify_id = secrets.token_urlsafe(24)
    ...
    def _approval_notify_sync(self, approval_data: dict) -> None:
        ...
        if _renders_exec_approval_buttons(type(adapter)):
            try:
                if approval_data.get("request_id") and _callable_accepts_keyword(adapter.send_exec_approval, "request_id"):
                    flags["request_id"] = approval_data["request_id"]
                fut = self._schedule(
