...
class GatewayTurnMixin:
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str, run_generation: int):
        ...
        session_entry = await self._apply_thechat_session_intent_async(event, source, session_entry)
    ...
    def _run_agent_display_settings(self, source: SessionSource) -> "GatewayRunner._RunAgentDisplay":
        ...
        interim_assistant_messages_enabled = not is_webhook and source.platform != Platform.THECHAT and interim_assistant_messages_mode != "off"
    ...
    def _thread_metadata_for_progress(
        ...
        if source.platform == Platform.SLACK and source.scope_id:
            ...
            metadata["slack_team_id"] = str(source.scope_id)
    ...
    def _run_agent_bind_turn_wiring(
        ...
        turn_ctx._structured_progress_adapter = turn_ctx._status_adapter
        turn_ctx._structured_progress_supported = (
            turn_ctx._status_adapter is not None
            and getattr(type(turn_ctx._status_adapter), "send_invocation_progress", BasePlatformAdapter.send_invocation_progress)
