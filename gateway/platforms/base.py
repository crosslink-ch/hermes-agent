...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    # TheChat routes late/async responses by the triggering message even when
    # no explicit task thread id was attached to the source.
...
class BasePlatformAdapter(ABC):
    ...
    async def _dispatch_inline_reply(self, event: MessageEvent, *, log_cmd: Optional[str] = None) -> bool:
    ...
    async def _dispatch_active_session_command(self, event: MessageEvent, session_key: str, cmd: str) -> None:
        ...
        try:
            ...
            # confirmation.
            await self._dispatch_inline_with_processing_lifecycle(event, session_key, cmd)
        ...
        if self._busy_session_handler is not None:
            try:
                if await self._busy_session_handler(event, session_key):
                    await self._complete_absorbed_active_session_event(event, session_key)
                    return
            except Exception as e:
    ...
    async def _dispatch_inline_with_processing_lifecycle(self, event: MessageEvent, session_key: str, cmd: str | None = None) -> None:
        """Close the bypass event's invocation without releasing the active turn's guard."""
        if not self._message_handler:
            return
        await self._run_processing_hook("on_processing_start", event)
        try:
            delivered = await self._dispatch_inline_reply(event, log_cmd=cmd)
        except Exception:
            await self._run_processing_hook("on_processing_complete", event, ProcessingOutcome.FAILURE)
            raise
        await self._run_processing_hook(
