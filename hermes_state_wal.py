...
def _apply_forced_journal_mode(
    ...
    *,
    ...
    actual = _set_journal_mode_no_wait(conn, mode.upper())
    if actual != mode:
...
def apply_wal_with_fallback(conn: sqlite3.Connection, *, db_label: str = "state.db", require_wal: bool = False) -> str:
    ...
    Except for an explicit HERMES_SQLITE_JOURNAL_MODE non-WAL override (verified via
    an exclusive no-wait transition), never downgrade if the on-disk header reports WAL or cannot be read — other
    ...
    if forced_mode is not None:
        configured = forced_mode
        if forced_mode != "wal":
            # Explicit override may leave WAL only with immediate exclusive ownership.
            # Never wait out a peer and sneak a transition between its writes.
            return _apply_forced_journal_mode(conn, forced_mode, db_label=db_label)
