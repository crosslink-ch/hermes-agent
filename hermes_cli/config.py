...
from hermes_cli.distribution import CONTAINER_IMAGE
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    the code; - the published Crosslink image bakes a ``docker`` stamp into
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": f"docker pull {CONTAINER_IMAGE}:latest",
...
_DOCKER_UPDATE_MESSAGE = f"""\
✗ ``hermes update`` doesn't apply inside the Docker container.

Hermes Agent runs as a published image ({CONTAINER_IMAGE}), not a
...
Verify the new version after restart:
  docker run --rm {CONTAINER_IMAGE}:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:crosslink-v2026.7.1``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/{CONTAINER_IMAGE}/tags
