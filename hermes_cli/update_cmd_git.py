...
from hermes_cli.distribution import (
    REPOSITORY_CANONICAL, REPOSITORY_HTTPS_URL, REPOSITORY_URLS,
    REPOSITORY_SLUG, canonical_github_remote,
)
...
_BAR = "=" * 68
_UPSTREAM_ADD_CMD = f"git remote add upstream {REPOSITORY_HTTPS_URL}"
...
def _is_fork(origin_url: Optional[str]) -> bool:
    ...
    return bool(origin_url) and canonical_github_remote(origin_url) != REPOSITORY_CANONICAL
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    print(
        "\nℹ Your fork is not tracking the Crosslink Hermes distribution.\n"
        f"  This means you may miss updates from {REPOSITORY_SLUG}.\n"
    )
