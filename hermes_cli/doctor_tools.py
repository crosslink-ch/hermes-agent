...
def _check_chromium() -> None:
    """agent-browser Chrome presence, using the exact predicate browser_tool uses to hide browser_* tools.
    ...
    if _is_camofox_mode() or bool(_get_cdp_override_raw()) or _get_cloud_provider() is not None or _using_lightpanda_engine():
        return
    if not check_bool(_chromium_installed(), ("agent-browser Chrome", "(browser engine)"),
                      ("agent-browser Chrome not installed", "(browser_* tools will be hidden from the agent)")):
        with_deps = "" if sys.platform == "win32" else "--with-deps "
        check_info(f"Install with: cd {PROJECT_ROOT} && npx agent-browser install {with_deps}")
