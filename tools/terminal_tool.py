...
def _active_turns_for_environment_key(environment_key: Hashable) -> int:
    """Read turn usage for an already scoped named-environment cache key."""
    turn_key = (
        environment_key[0]
        if isinstance(environment_key, tuple) and len(environment_key) == 2
        else _turn_scope_key(environment_key)
    )
    with _active_turn_counts_lock:
        return _active_turn_counts.get(turn_key, 0)
...
def _environment_replacement_is_busy(env: Any, environment_key: Hashable) -> bool:
    ...
    if not _environment_has_stable_storage(env):
        return False
    if _active_turns_for_environment_key(environment_key) > 1:
        return True
    try:
                    ...
                    try:
                        ...
                        if env_type in {"docker", "singularity", "modal", "daytona"}:
                            container_config = {
                                ...
                                "storage_task_id": target_resolution.storage_task_id(
                                    effective_base_task_id
                                ),
                                "legacy_storage_task_id": (
                                    target_resolution.legacy_backend_task_id(
                                        effective_base_task_id
                                    )
                                ),
                            }
