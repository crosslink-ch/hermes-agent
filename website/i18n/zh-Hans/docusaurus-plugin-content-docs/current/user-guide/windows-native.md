...
- 与 `agent-browser` 匹配的 Chrome-for-Testing 会在首次运行时自动安装（`npx agent-browser install`）。如果安装失败，`hermes doctor` 会给出修复提示。
...
**浏览器工具启动了，但工具调用超时。**
Chrome 会在首次运行时自动安装。如果安装失败，运行 `hermes doctor`——它会检测缺失的浏览器并打印修复所需的确切 `npx agent-browser install` 命令。
