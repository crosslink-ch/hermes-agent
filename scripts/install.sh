...
EOF
        ...
        # Crosslink is public: HTTPS first, with upstream throttling recovery.
        ...
        if [ "$clone_ok" = true ]; then
            ...
            if GIT_SSH_COMMAND="ssh -o BatchMode=yes -o ConnectTimeout=5" \
               git clone --depth 1 --single-branch --branch "$BRANCH" "$REPO_URL_SSH" "$INSTALL_DIR" 2>/dev/null; then
                log_success "Cloned via SSH"
            else
                rm -rf "$INSTALL_DIR" 2>/dev/null  # failed fresh clone is unusable
