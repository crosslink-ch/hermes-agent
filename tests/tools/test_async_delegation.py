...
def test_durable_store_honors_forced_delete_on_existing_wal_db(tmp_path, monkeypatch):
    """The async registry must preserve the owning SessionDB journal policy.
    ...
    with closing(sqlite3.connect(tmp_path / "state.db", isolation_level=None)) as seed:
        assert seed.execute("PRAGMA journal_mode=WAL").fetchone()[0] == "wal"

    # The long-lived owner applies policy once; short-lived ledger handles inherit
    # it and must not force WAL or downgrade underneath a live SessionDB writer.
...
def test_connect_closes_new_connection_when_wal_policy_initialization_fails(
    ...
    monkeypatch.setattr(hermes_state_repair, "apply_durability_barriers", fail_wal_policy)
