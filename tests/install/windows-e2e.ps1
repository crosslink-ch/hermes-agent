...
param(
    ...
    [string]$SetupExeUrl = "https://github.com/crosslink-ch/hermes-agent/releases/latest/download/Hermes-Setup.exe",
...
$RepoUrlHttps = "https://github.com/crosslink-ch/hermes-agent.git"
$RepoUrlSsh   = "git@github.com:crosslink-ch/hermes-agent.git"
...
if /I "!prev2!"=="remote" if /I "!prev1!"=="get-url" if /I "%~1"=="origin" (
    set "originalArgs=%*"
    set "originArgs=!originalArgs:remote get-url origin=config --get remote.origin.url!"
    `"$realGit`" !originArgs!
    exit /b !ERRORLEVEL!
)
set prev2=!prev1!
...
"@ | Set-Content -LiteralPath $shimPath -Encoding ASCII
    ...
    $ErrorActionPreference = $prevEap
    $expectedOrigin = Invoke-Git @("-C", $RepoRoot, "config", "--get", "remote.origin.url")
    Assert-True ($observedGitUrl -eq $expectedOrigin) "git remote get-url shim: origin resolves to '$observedGitUrl', expected '$expectedOrigin'"
...
function Invoke-RefInstaller {
    ...
    if ($text -match '\$MigrateLegacyOrigin') { $flags += "-MigrateLegacyOrigin" }
...
function Invoke-PhaseStage {
    ...
    if (-not $oldRef -or $oldRef -eq "auto") {
        ...
        $tagList = Invoke-Git @("-C", $RepoRoot, "tag", "--list", "crosslink-v[0-9]*", "--sort=-creatordate")
        if (-not $tagList) {
            $tagList = Invoke-Git @("-C", $RepoRoot, "tag", "--list", "v[0-9]*", "--sort=-creatordate")
        }
