...
The fork samples `crosslink-vYYYY.M.D[.N]` releases when present, rather than
letting inherited upstream tags displace them. An explicit `install-ref` accepts
both release namespaces so legacy migration remains testable.

All git-redirect drivers cover both Crosslink and legacy Nous URLs. Their origin
probe reports the checkout's stored `remote.origin.url`, not a fabricated
Crosslink URL: this allows the current installer to exercise its real migration
guard. Drivers pass `--migrate-legacy-origin` / `-MigrateLegacyOrigin` only if the
selected ref's installer supports it. Unknown/custom origins are never retargeted.
