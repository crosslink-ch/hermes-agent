...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://github.com/crosslink-ch/hermes-agent/releases/latest/download/Hermes-Setup.dmg"
...
for ((i=0; i+2<\${#args[@]}; i++)); do
    if [ "\${args[i]}" = remote ] && [ "\${args[i+1]}" = get-url ] && [ "\${args[i+2]}" = origin ]; then
        exec "$REAL_GIT" "\${args[@]:0:i}" config --get remote.origin.url
    fi
done
...
EOF
  ...
  # check it worked
  expected_origin="$("$REAL_GIT" -C "$REPO_ROOT" config --get remote.origin.url)"
  observed_git_url="$(git -C "$REPO_ROOT" remote get-url origin)"
  if [[ "$observed_git_url" != "$expected_origin" ]]; then
    fail "failed git remote get-url shim: origin resolves to '$observed_git_url', expected '$expected_origin'"
  fi
  ok "git remote get-url shim: $SHIM_DIR/git -> $REAL_GIT (origin reports its actual configured URL)"
...
phase_stage() {
  ...
  if [ -z "$old_ref" ] || [ "$old_ref" = "auto" ]; then
    old_ref="$(git -C "$REPO_ROOT" tag --list 'crosslink-v[0-9]*' --sort=-creatordate | head -1)"
    if [ -z "$old_ref" ]; then
...
run_installer() {
  ...
  # Explicit consent only when the selected installer understands migration.
  # Older releases have no such flag; custom origins stay protected by the installer.
  if installer_supports "$1" "--migrate-legacy-origin"; then
    flags+=(--migrate-legacy-origin)
  fi
