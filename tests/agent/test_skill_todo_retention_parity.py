...
class TestSkillGuidanceSurvivesWithTodos:
    ...
    def test_reload_instruction_travels_with_todo_snapshot(self, tmp_path):
        ...
        note_rows = [
            row for row in compressed
            if row.get("_todo_snapshot_internal") is True
        ]
        assert len(note_rows) == 1
        note_text = str(note_rows[0]["content"])
        assert TODO_INJECTION_HEADER in note_text
        assert "Remove the Lightning screen" in note_text
        # The fork keeps continuity as assistant-authored bookkeeping while
        # retaining upstream's pruned-skill recovery notice beside it.
        assert _PRUNED_SKILL_RELOAD_NOTICE_HEADER in note_text
        assert "skill_view(name='hodle-design-system')" in note_text
        assert note_text.index(TODO_INJECTION_HEADER) < note_text.index(
            _PRUNED_SKILL_RELOAD_NOTICE_HEADER
        )
    ...
    def test_no_notice_when_no_skills_pruned(self, tmp_path):
        ...
        note_text = str(note_rows[0]["content"])
        assert TODO_INJECTION_HEADER in note_text
        assert _PRUNED_SKILL_RELOAD_NOTICE_HEADER not in note_text
    ...
    def test_synthetic_row_classification_unbroken(self, tmp_path):
        ...
        assert row.get("_todo_snapshot_internal") is True
        assert row["role"] == "assistant"
        assert not _is_real_user_message(row)
        assert not ContextCompressor._is_synthetic_compression_user_turn(row)
...
class TestNoticeStripLifecycle:
    ...
    def test_repeated_boundaries_keep_single_notice(self, tmp_path):
        ...
        assert len(note_rows) == 1
        note_text = str(note_rows[0]["content"])
        assert note_text.count(TODO_INJECTION_HEADER) == 1
        assert note_text.count(_PRUNED_SKILL_RELOAD_NOTICE_HEADER) == 1
        assert "stale task" not in note_text
        assert "stale-skill" not in note_text
        assert "fresh task" in note_text
        assert "skill_view(name='hodle-design-system')" in note_text
        assert compressed[-1]["content"] == "keep this human text"
