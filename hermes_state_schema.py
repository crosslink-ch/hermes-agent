...
class SessionSchemaMixin:
    ...
    def _detach_stale_fts_if_marked(self, cursor: sqlite3.Cursor) -> bool:
        """Detach marked indexes atomically without write-locking a settled open."""
        # A healthy open needs no writer admission. If a marker is present, re-read
        # it under BEGIN IMMEDIATE: a peer may have rebuilt and restored triggers
        # since this read, and stale local state must never detach those triggers.
        if cursor.execute(
    ...
    def _recover_stale_fts_locked(self, cursor: sqlite3.Cursor, *, legacy: bool) -> bool:
        ...
        try:
            ...
            try:
                ...
                trigram_status = True
            include_trigram = trigram_status is True or (
                not legacy and self._trigram_tokenizer_available(cursor)
            )
            ...
            self._execute_ddl_script_transactional(cursor, schema_sql)
            self._rebuild_fts_indexes(cursor, legacy=legacy, include_trigram=include_trigram)
            if not legacy:
                cursor.execute(_DROP_RETIRED_TOOL_HIGH_WATER_SQL)
            cursor.execute(
    ...
    def _init_schema(self):
        ...
        # Detach marked derived state before reconciliation can update canonical messages.
        ...
        # A peer may have entered fail-open during non-FTS reconciliation.
        self._fts_stale = self._detach_stale_fts_if_marked(cursor)
    ...
    def _run_data_migrations(self, cursor: sqlite3.Cursor, current_version: int, fts5_available: bool) -> None:
        ...
        fts_migrations_complete = not self._fts_stale
        if current_version < 30 and fts5_available and not self._fts_stale:
